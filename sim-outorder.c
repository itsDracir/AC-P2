...
/* yags predictor config (<GBHRsize> <PHTsize> <GBHRwidth> <xor>) */
static int yags_nelt = 4;
static int yags_config[4] =
  {/* GBHRsize */1, /* PHTsize */4, /* GBHRwidth */ 1, /* xor */FALSE};
		  ...
		  int argc, char **argv)        /* command line arguments */
{
  ...
  if (!mystricmp(pred_type, "perfect"))
    ...
    {
      if (yags_nelt != 4) 
    fatal("bad 2-level pred config (<GBHRsize> <PHTsize> <GBHRwisth> <xor>)");
      if (btb_nelt != 2)
    fatal("bad btb config (<num_sets> <associativity>)");
